<?php
/**
 * The library responsible for the UI of all BLYD3D products.
 *
...
class Kali_Admin_Tools {
	...
	public static function register_custom_post_type( $name, $slug, $desc = '' ) {
		$labels = array(
			'name'               => __( $name, 'blyd3d' ),
			'singular_name'      => __( $name, 'blyd3d' ),
			'add_new'            => __( 'New ' . $name, 'blyd3d' ),
			'add_new_item'       => __( 'New ' . $name, 'blyd3d' ),
			'edit_item'          => __( 'Edit ' . $name, 'blyd3d' ),
			'new_item'           => __( 'New ' . $name, 'blyd3d' ),
			'view_item'          => __( 'View group', 'blyd3d' ),
			'not_found'          => __( 'No ' . $name . ' found', 'blyd3d' ),
			'not_found_in_trash' => __( 'No  ' . $name . ' in the trash', 'blyd3d' ),
			'menu_name'          => __( $name, 'blyd3d' ),
		);
		...
		foreach ( $options as $value ) {
			...
			switch ( $value['type'] ) {
				...
				case 'repeatable-fields':
					...
					$add_label = self::get_parse_value( $value, 'add_btn_label', __( 'Add', 'blyd3d' ) );
					?>
							</tbody>
						</table>
						<a class="button mg-top kali-add-new-row" data-tpl2="[{<?php echo esc_attr( $tpl_id ); ?>}]"
	...
	public static function get_repeatable_rows( $repeatable_field_settings, $repeatable_fields_data ) {
		...
		?>
		<input class="kali-row-count" type="hidden" value="<?php echo esc_attr( $row_compt ); ?>">
		<?php
	}
	...
	public static function get_image_url( $suspected_link, $with_root = true ) {
		...
		$root = str_replace( 'https', 'http', self::get_root_url( '/' ) );
		$img_src = str_replace( self::get_root_url( '/' ), '', $img_src );
		$img_src = str_replace( $root, '', $img_src );
	...
	public static function afficherMedia($url) {
		$extension = pathinfo($url, PATHINFO_EXTENSION);

		if (preg_match("/^jpg|png|webp|jpeg|gif$/i", $extension)) {
			echo '<img src="'.$url.'" alt="Image">';
		} elseif (preg_match("/^mp4|webm|ogg$/i", $extension)) {
			echo '<video width="40" height="40" src="'.$url.'" type="video/'.$extension.'">
				</video>';
		} else {
			echo '';
		}
